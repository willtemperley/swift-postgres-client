...
struct PostgresTimeWithTimeZoneTest {
    ...
    func test() {
        ...
        #expect(pacificTimeZone.nextDaylightSavingTimeTransition != nil)
        #expect(PostgresTimeWithTimeZone(
            hour: 1, minute: 60, second: 1, nanosecond: 1, timeZone: pacificTimeZone) == nil)
        
        // Invalid component values should fail.
        #expect(PostgresTimeWithTimeZone(
            hour: 1, minute: 61, second: 1, nanosecond: 1, timeZone: utcTimeZone) == nil)
        ...
        #expect(PostgresTimeWithTimeZone("foo") == nil)

        // Invalid string representation should fail.
        #expect(PostgresTimeWithTimeZone("01:61:01.000+00:00") == nil)
    ...
    func checkTime(
        ...
        func checkTime(_ t: PostgresTimeWithTimeZone) {
            ...
            #expect(t == time)
            #expect(isValidDate(tDateComponents))
            assertApproximatelyEqual(tDateComponents, expectedDateComponents)
            assertApproximatelyEqual(tDate, expectedDate)
            #expect(tTimeZone == expectedTimeZone)
            #expect(tPostgresValue == expectedPostgresValue)
            #expect(tDescription == expectedDescription)
        }
