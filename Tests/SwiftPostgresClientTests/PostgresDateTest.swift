...
struct PostgresDateTest {
    ...
    @Test func test() {
        ...
        #expect(PostgresDate(year: 2019, month: 0, day: 1) == nil)
        
        // Invalid component values should fail.
        #expect(PostgresDate(year: 2019, month: 2, day: 29) == nil)
        ...
        #expect(PostgresDate("foo") == nil)
        
        // Invalid string representation should fail.
        #expect(PostgresDate("2019-00-01") == nil)
    ...
    func checkPostgresDate(
        ...
        _ expectedDescription: String) {
            ...
            func checkPostgresDate(_ pd: PostgresDate) {
                ...
                #expect(pd == postgresDate)
                #expect(isValidDate(pdDateComponents))
                assertApproximatelyEqual(pdDateComponents, expectedDateComponents)
                assertApproximatelyEqual(pdUtcDate, expectedUtcDate)
                assertApproximatelyEqual(pdPacificDate, expectedPacificDate)
                #expect(pdPostgresValue == expectedPostgresValue)
                #expect(pdDescription == expectedDescription)
            }
